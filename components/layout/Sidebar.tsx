    ...
    <aside className="hidden lg:flex fixed inset-y-0 left-0 w-60 flex-col border-r border-line bg-surface/50 backdrop-blur-xl z-30">
      ...
      <nav ref={navRef} className="relative flex-1 px-3 py-4 space-y-1">
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <span className={cn("transition-colors duration-200", active ? "text-accentInk" : "")}>{item.icon}</span>
              {item.label}
            </Link>
          );
        })}
        ...
        >
          <span className={cn("transition-colors", pathname === "/docs" ? "text-accentInk" : "")}>
        ...
        >
          <span className={cn("transition-colors", pathname === "/extend" ? "text-accentInk" : "")}>
            <svg viewBox="0 0 18 18" className="h-[18px] w-[18px]" fill="none" aria-hidden="true">
      ...
      <div className="px-3 py-3 border-t border-line space-y-1">
        <button
          ...
          className="w-full flex items-center gap-3 h-9 px-3 rounded-input text-13 font-medium text-ink-faint hover:text-accentInk hover:bg-elevate/[0.025] transition-colors"
        >
        ...
        <a
          ...
          className="flex items-center gap-3 h-9 px-3 rounded-input text-13 text-ink-faint hover:text-accentInk hover:bg-elevate/[0.025] transition-colors"
        >
          <svg viewBox="0 0 16 16" className="h-4 w-4" fill="none" aria-hidden="true">
